...
const TutorDetails = () => {
 const {user} = use(AuthContext);
  ...
  const { userName, price, language, description, image, book,email, rating } = tutorial;
  ...
  const handleBook = () => {
   ...
   //handle book toggle api fetch call
  }
  return (
    <>
      <div className="flex border border-fuchsia-500 rounded-xl shadow-lg p-6 items-center space-x-6 max-w-4xl mx-auto my-6 bg-white hover:bg-fuchsia-50 transition duration-300">
        ...
        <div className="flex-1">
          ...
          <button onClick={handleBook} className="bg-fuchsia-500 hover:bg-fuchsia-600 text-white px-5 py-2.5 rounded-md text-sm shadow-md transition cursor-pointer w-full">
            Book Now
          </button>
        </div>
      </div>
